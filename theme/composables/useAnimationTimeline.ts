...
export function useAnimationTimeline(
  ...
  function skipToEnd() {
    // Fast-forward the animation to its final state
    clearAllTimers()

    // Ensure user text is marked as sent and assistant shows last frame
    typedInputText.value = ''
    sentUserText.value = config.userPrompt
    currentFrameIndex.value = Math.max(0, config.totalFrames - 1)
    showCursor.value = false
    currentPhase.value = 'done'
    scrollToBottom()
  }
